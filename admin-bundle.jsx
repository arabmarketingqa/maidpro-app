...
const HourlySection = ({ store, set }) => {
  ...
  const addService = () => set({ services: [...store.services, { id: `sv${Date.now()}`, name: "New service", emoji: "🧽", rate: 15, on: true }] });
...
const MonthlySection = ({ store, set }) => {
  ...
  const addMonthly = () => set({ monthly: [...store.monthly, { id: `pkg${Date.now()}`, name: "New Package", emoji: "📦", maids: 1, daysPerWeek: 4, hoursPerDay: 4, priceMonthly: 1000, discountLabel: "" }] });
...
const CalendarSection = ({ store, set, bookings }) => {
  ...
  return (
    <div className="space-y-5 fade-up">
      {/* Daily staff schedule — primary view */}
...
const parseHour = (t) => { if (!t || t === "—") return null; const [h,m] = t.split(":").map(Number); return isNaN(h) ? null : h + (m||0)/60; };
                ...
                {todays.filter(b => (store.assignments?.[b.ref] || []).includes(s.id)).map(b => {
                  ...
                  return (
                    <div key={`${b.ref}-${s.id}`}
                      ...
                      <div className="text-[10.5px] font-mono opacity-80">{(() => { const h = Math.floor(start), m = Math.round((start-h)*60); const e = start + b.hours; const eh = Math.floor(e), em = Math.round((e-eh)*60); return `${fmt12(h,m)} – ${fmt12(eh,em)}`; })()}</div>
