/* Pricing model + live calculator card */

const SERVICE_TYPES = [
  { id: "regular", name: "Regular Cleaning",  emoji: "🧹", rate: 15, desc: "Routine tidy-up: dusting, mopping, surfaces" },
  { id: "deep",    name: "Deep Cleaning",     emoji: "✨", rate: 15, desc: "Top-to-bottom, including kitchen + baths" },
  { id: "movein",  name: "Move-in / Out",     emoji: "📦", rate: 15, desc: "Empty-property turnover before keys swap" },
  { id: "post",    name: "Post-Construction", emoji: "🏗️", rate: 15, desc: "Dust, debris & paint residue removal" },
];

const HOURLY_BASE_FEE = 0;     // QAR
const MATERIALS_PER_HOUR = 10; // QAR/hr
const MAID_MULTIPLIER = 1;     // each maid bills full rate × hours
...
const MONTHLY_RATE_PER_HOUR = 15; // QAR per maid hour for custom calc
const WEEKS_IN_MONTH = 4;
const MONTHLY_DISCOUNT = 0.10; // 10% off custom monthly
...
function natRate(state) {
  const n = NATIONALITIES.find(x => x.id === state.nationality) || NATIONALITIES[0];
  return n.rate;
}

function computePrice(state) {
  const type = SERVICE_TYPES.find(t => t.id === state.serviceType) || SERVICE_TYPES[0];
  const rate = natRate(state);
  let hours = state.hours;
  let maids = state.maids;
  let visits = 1;

  if (state.mode === "stayin") {
    const pkg = STAYIN_PACKAGES.find(p => p.id === state.stayInId) || STAYIN_PACKAGES[0];
    return {
      rate: 0, hours: 24, maids: 1, visits: pkg.months,
      labour: pkg.price, materials: 0, subtotal: pkg.price,
      monthlyDiscount: pkg.save || 0,
      total: pkg.price,
  ...
  if (state.mode === "monthly") {
    const pkg = MONTHLY_PACKAGES.find(p => p.id === state.packageId) || MONTHLY_PACKAGES[0];
    if (pkg.custom) {
      maids = state.customMaids || 1;
      const days = state.customDays || 4;
      hours = state.customHours || 4;
      const subtotal = rate * hours * maids * days * WEEKS_IN_MONTH;
      const monthlyDiscount = Math.round(subtotal * MONTHLY_DISCOUNT);
      const materials = state.materials ? MATERIALS_PER_HOUR * hours * days * WEEKS_IN_MONTH : 0;
      const total = subtotal - monthlyDiscount + materials;
      return {
        rate, hours, maids, visits: days * WEEKS_IN_MONTH,
    ...
    // preset monthly: rate × maids × days/wk × 4 wk × hrs/day
    maids = pkg.maids;
    hours = pkg.hoursPerDay;
    visits = pkg.daysPerWeek * WEEKS_IN_MONTH;
    const labour = rate * hours * maids * visits;
    const materials = state.materials ? MATERIALS_PER_HOUR * hours * visits : 0;
    return {
      rate, hours, maids, visits,
      labour, materials, subtotal: labour + materials,
      monthlyDiscount: 0,
  ...
  // hourly
  const labour = rate * hours * maids;
  const materials = state.materials ? MATERIALS_PER_HOUR * hours : 0;
  const subtotal = labour + materials;
  return {
    rate, hours, maids, visits: 1,
...
const PriceCard = ({ state, breakdown, step, onPrimary, primaryLabel, primaryDisabled, compact }) => {
  const { rate, hours, maids, visits, labour, materials, subtotal, monthlyDiscount, total, serviceName } = breakdown;
  return (
    <aside className={`bg-white rounded-xl2 hairline shadow-card overflow-hidden ${compact ? "" : ""}`}>
      ...
      <div className="px-5 py-4">
        ...
        {state.materials && <Row label="Cleaning materials" sub={`${MATERIALS_PER_HOUR} QAR/hr`} value={<Money value={materials} />} />}
...
Object.assign(window, { SERVICE_TYPES, MONTHLY_PACKAGES, STAYIN_PACKAGES, NATIONALITIES, MATERIALS_PER_HOUR, natRate, computePrice, PriceCard, Money });
